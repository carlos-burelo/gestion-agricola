...
export default async function PrestamosBancariosPage() {
  const actor = await getCurrentUser()
  if (!actor) redirect("/login")

  const config = getModuleBySlug("prestamos-bancarios")!
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Entradas de dinero otorgadas por instituciones bancarias y créditos de financiamiento."
      >
        <RecordForm
          config={config}
          ...
          trigger={
            <Button className="gap-1.5 shadow-xs">
              ...
              <span>Nuevo Préstamo Bancario</span>
            </Button>
          }
        />
      </PageHeader>
