...
export default async function AbonosClientesPage() {
  const actor = await getCurrentUser()
  if (!actor) redirect("/login")

  const config = getModuleBySlug("abonos-clientes")!
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Cobros y abonos aplicados a ventas de piña a crédito (Cuentas por Cobrar)."
      >
        <RecordForm
          config={config}
          ...
          trigger={
            <Button className="gap-1.5 shadow-xs">
              ...
              <span>Nuevo Abono de Cliente</span>
            </Button>
          }
        />
      </PageHeader>
